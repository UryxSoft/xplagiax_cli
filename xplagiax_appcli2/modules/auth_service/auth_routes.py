...
# Rate limiting (basic implementation - use Flask-Limiter in production)
_rate_limit_storage = {}
...
def record_login(user, session_token):
    """Insert a row into logins_xplagiax_clients after a successful login."""
    try:
...
def recent_sessions():
    ...
    result = []
    for r in rows:
        # Determinar si es la sesión activa o un logout (session_token=None indica que fue invalidada)
        is_current = (r.session_token is not None and
                      r.session_token == current_user.session_token)
        event_type = getattr(r, 'event_type', None) or ('login' if r.session_token else 'logout')
        result.append({
            'id': r.id,
...
def login_history_chart():
    ...
    # Contar filas: las que tienen session_token son logins activos;
    # las que no (token=None tras invalidación) se tratan como logouts.
    rows = (db.session.query(
                func.date(LoginHistory.created_at).label('d'),
                func.count(LoginHistory.id).label('total'),
                func.sum(
                    sa_case(
                        (LoginHistory.session_token.isnot(None), 1),
                        else_=0
                    )
                ).label('logins_count'),
                func.sum(
                    sa_case(
                        (LoginHistory.session_token.is_(None), 1),
                        else_=0
                    )
                ).label('logouts_count'))
            ...
            .all())
    login_map  = {str(r.d): int(r.logins_count  or 0) for r in rows}
    logout_map = {str(r.d): int(r.logouts_count or 0) for r in rows}
    labels, logins, logouts = [], [], []
    for i in range(13, -1, -1):
        day = datetime.utcnow() - timedelta(days=i)
        key = day.strftime('%Y-%m-%d')
        labels.append(day.strftime('%b %d'))
        logins.append(login_map.get(key, 0))
        logouts.append(logout_map.get(key, 0))
    return jsonify({'labels': labels, 'logins': logins, 'logouts': logouts})
