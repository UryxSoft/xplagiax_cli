...
def oauth_callback(provider):
    ...
    try:
        ...
        if save_user_tokens(session['user_id'], user_tokens):
            ...
            # Volver a home y reabrir Settings > Integrations con toast
            return redirect('/?openSettings=integrations&connected=' + provider)
        else:
...
@x_integ.route('/storage/preview/<provider>/<path:file_id>')
@login_required
def cloud_doc_preview(provider, file_id):
    """Download first bytes of a cloud file and return a text preview."""
    user_id = session['user_id']
    ...
    try:
        if provider == 'native':
            ...
            from settings.connections import db as _db
            f = FileModel.query.filter_by(id=file_id, user_id=user_id).first()
            if not f:
                return jsonify({'preview': ''})
            resp = requests.get(f.minio_url, timeout=12, stream=True)
            content = _read_limited(resp, MAX_BYTES)
            filename = filename or f.original_filename
        else:
            user_tokens = get_user_tokens(user_id)
            if provider not in user_tokens:
                return jsonify({'preview': ''})
            ...
            if is_token_expired(token_info):
                if not refresh_access_token(user_id, provider):
                    return jsonify({'preview': ''})
                token_info = get_user_tokens(user_id)[provider]
            content = _download_cloud_preview(provider, file_id, token_info['access_token'], MAX_BYTES)

        preview = _extract_text_preview(content, filename, 220)
        return jsonify({'preview': preview})
    except Exception as e:
        print(f'doc-preview error [{provider}/{file_id}]: {e}')
        return jsonify({'preview': ''})
...
def _download_cloud_preview(provider, file_id, access_token, max_bytes):
    auth = {'Authorization': f'Bearer {access_token}'}
    try:
        if provider == 'google_drive':
            resp = requests.get(
                f'https://www.googleapis.com/drive/v3/files/{file_id}?alt=media',
                headers=auth, timeout=15, stream=True)
            if resp.status_code in (400, 403):
                # Native Google Doc — export as plain text instead
                resp = requests.get(
            ...
            resp = requests.post(
                ...
                headers={**auth, 'Dropbox-API-Arg': json.dumps({'path': file_id})},
                timeout=15, stream=True)
        elif provider == 'box':
            resp = requests.get(
                f'https://api.box.com/2.0/files/{file_id}/content',
                headers=auth, timeout=15, stream=True, allow_redirects=True)
        elif provider == 'onedrive':
            resp = requests.get(
                f'https://graph.microsoft.com/v1.0/me/drive/items/{file_id}/content',
                headers={**auth, 'Range': f'bytes=0-{max_bytes - 1}'},
                timeout=15, stream=True, allow_redirects=True)
        else:
            return b''
        if resp.status_code not in (200, 206):
            print(f'Cloud preview HTTP {resp.status_code} for {provider}/{file_id}')
            return b''
        return _read_limited(resp, max_bytes)
    except Exception as e:
        print(f'Cloud download error [{provider}/{file_id}]: {e}')
        return b''


def _extract_text_preview(content, filename, max_chars):
    ...
    try:
        if ext == 'pdf':
            ...
            for i, page in enumerate(doc):
                ...
                if i >= 2 or sum(len(p) for p in parts) > max_chars * 4:
                    break
            doc.close()
            text = ' '.join(parts)
        elif ext == 'docx':
            ...
            for para in doc.paragraphs:
                ...
                if t:
                    parts.append(t)
                if sum(len(p) for p in parts) > max_chars * 4:
                    break
            text = ' '.join(parts)
